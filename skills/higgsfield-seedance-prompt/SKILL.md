...
For a multi-stage story-video project, use [the two-pass project contract](references/project-two-pass.md). In that mode the first pass writes a standalone draft prompt and a separate machine-readable reference request; the second pass binds approved image files and writes the final standalone prompt plus provider input map. The standalone prompt rule still governs each prompt file.
